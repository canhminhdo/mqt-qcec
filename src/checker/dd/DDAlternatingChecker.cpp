...
namespace ec {
    void DDAlternatingChecker::initialize() {
        ...
        // Only count ancillaries that are present in but not acted upon in both
        // of the circuits. Otherwise, the alternating checker must not be used.
        ...
        for (qc::Qubit q = 0U; q < nqubits; ++q) {
            ancillary[static_cast<std::size_t>(q)] =
                qc1.logicalQubitIsAncillary(q) &&
                qc2.logicalQubitIsAncillary(q);
        }

        // reduce the ancillary qubit contributions
        // [1 0] if the qubit is no ancillary, or it is acted upon by both
        // circuits [0 1]
        //
    ...
    void DDAlternatingChecker::execute() {
        while (!taskManager1.finished() && !taskManager2.finished() &&
            ...
            if (!taskManager1.finished() && !taskManager2.finished()) {
                ...
                // whenever the current functionality resembles the identity,
                // identical gates on both sides cancel
    ...
    void DDAlternatingChecker::postprocess() {
        // ensure that the permutations that were tracked throughout the circuit
        // match the expected output permutations
    ...
    bool DDAlternatingChecker::canHandle(const qc::QuantumComputation& qc1,
        ...
        for (auto qubit =
                 static_cast<std::make_signed_t<qc::Qubit>>(nqubits - 1U);
             qubit >= 0; --qubit) {
