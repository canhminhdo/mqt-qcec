...
namespace ec {
    ...
    DDEquivalenceChecker<DDType, Config>::equals(const DDType& e,
                                                 const DDType& f) {
        // both node pointers being equivalent is the strongest indication that
        // the two decision diagrams are equivalent
        if (e.p == f.p) {
            // whenever the top edge weights differ, both decision diagrams are
        ...
        // in general, decision diagrams are canonic. This implies that if their
        // top nodes differ, they are not equivalent. However, numerical
        // instabilities might create a scenario where two nodes differ besides
        // their underlying decision diagrams being extremely close (for some
        // definition of `close`).
        if constexpr (std::is_same_v<DDType, qc::MatrixDD>) {
            // for matrices this can be resolved by calculating their Frobenius
            // inner product trace(U V^-1) and comparing it to some threshold.
            // in a similar fashion, we can simply compare U V^-1 with the
            // identity, which results in a much simpler check that is not prone
            // to overflow.
            bool       isClose{};
            const bool eIsClose = dd->isCloseToIdentity(
                e, configuration.functionality.traceThreshold);
            const bool fIsClose = dd->isCloseToIdentity(
                f, configuration.functionality.traceThreshold);
            if (eIsClose || fIsClose) {
                // if any of the DDs is close to the identity (structure), the
                // result can be decided by whether both DDs are close enough to
                // the identity.
                isClose = eIsClose && fIsClose;
            } else {
                // otherwise, one DD needs to be inverted before multiplying
                // both of them together and checking whether the resulting DD
            ...
            if (isClose) {
                // whenever the top edge weights differ, both decision diagrams
                // are only equivalent up to a global phase
                if (!e.w.approximatelyEquals(f.w)) {
                    return EquivalenceCriterion::EquivalentUpToGlobalPhase;
                }
                return EquivalenceCriterion::Equivalent;
            ...
            // whenever |<e,f>|^2 ≃ 1, both decision diagrams should be
            // considered equivalent up to a phase
            const auto fidelity = (innerProduct.r * innerProduct.r) +
                                  (innerProduct.i * innerProduct.i);
            if (std::abs(fidelity - 1.0) <
                configuration.simulation.fidelityThreshold) {
                return EquivalenceCriterion::EquivalentUpToPhase;
            }
        }
    ...
    EquivalenceCriterion DDEquivalenceChecker<DDType, Config>::run() {
        ...
        // initialize the internal representation (initial state, initial
        // matrix, etc.)
        initialize();
    ...
    void DDEquivalenceChecker<DDType, Config>::postprocessTask(
        ...
        // ensure that the permutation that was tracked throughout the circuit
        // matches the expected output permutation
        task.changePermutation();
        if (isDone()) {
            return;
        }
        // eliminate the superfluous contributions of ancillary qubits (this
        // only has an effect on matrices)
        task.reduceAncillae();
        if (isDone()) {
            return;
        }
    ...
    template <class DDType, class Config>
    EquivalenceCriterion
    DDEquivalenceChecker<DDType, Config>::checkEquivalence() {
    ...
    void DDEquivalenceChecker<DDType, Config>::initializeApplicationScheme(
        ...
        case ApplicationSchemeType::Lookahead:
            if constexpr (std::is_same_v<DDType, qc::MatrixDD>) {
                applicationScheme =
                    std::make_unique<LookaheadApplicationScheme<Config>>(
                        taskManager1, taskManager2);
            } else {
                throw std::invalid_argument("Lookahead application scheme can "
                                            "only be used for matrices.");
            }
            break;
