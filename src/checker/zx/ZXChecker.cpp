...
namespace ec {
    ZXEquivalenceChecker::ZXEquivalenceChecker(
        const qc::QuantumComputation& circ1,
        const qc::QuantumComputation& circ2, Configuration config) noexcept
        ...
        for (auto anc = static_cast<zx::Qubit>(qc1.getNqubits() - 1U);
            ...
            miter.makeAncilla(anc, static_cast<zx::Qubit>(
                                       p1.at(static_cast<qc::Qubit>(anc))));
            dPrime.makeAncilla(anc, static_cast<zx::Qubit>(
                                        p2.at(static_cast<qc::Qubit>(anc))));
        }
        miter.invert();
        miter.concat(dPrime);
    }
    ...
    EquivalenceCriterion ZXEquivalenceChecker::run() {
        ...
        // non-equivalence might be due to incorrect assumption about the state
        // of ancillaries or the check was aborted prematurely, so no
