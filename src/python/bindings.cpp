...
namespace ec {
    namespace {
        ...
        createManagerFromConfiguration(
    ...
    PYBIND11_MODULE(pyqcec, m) {
        m.doc() =
            "Python interface for the MQT QCEC quantum circuit equivalence "
            "checking tool";
        ...
        py::enum_<ApplicationSchemeType>(m, "ApplicationScheme")
            .value("sequential", ApplicationSchemeType::Sequential,
                   "Applies all gates from the first circuit, before "
                   "proceeding with "
            ...
            .value(
                ...
                "applied from the second circuit according to the difference "
                "in "
                "the number of gates.")
            .value("lookahead", ApplicationSchemeType::Lookahead,
                   "Looks whether an application from the first circuit or the "
                   "second "
                   "circuit yields the smaller decision diagram. Only works "
                   "for the "
            ...
            .value("gate_cost", ApplicationSchemeType::GateCost,
                   "Each gate of the first circuit is associated with a "
                   "corresponding "
                   "cost according to some cost function *f(...)*. Whenever a "
                   "gate *g* "
                   "from the first circuit is applied *f(g)* gates are applied "
                   "from the "
        ...
        py::enum_<StateType>(m, "StateType")
            .value("computational_basis", StateType::ComputationalBasis,
                   "Randomly choose computational basis states. Also referred "
                   "to as "
                   "*classical*.")
            .value("random_1Q_basis", StateType::Random1QBasis,
                   "Randomly choose a single-qubit basis state for each qubit "
                   "from "
                   "the six-tuple *(|0>, |1>, |+>, |->, |L>, |R>)*. Also "
                   "referred to "
                   "as *local_random*.")
            .value("stabilizer", StateType::Stabilizer,
                   "Randomly choose a stabilizer state by creating a random "
                   "Clifford "
        ...
        py::enum_<EquivalenceCriterion>(m, "EquivalenceCriterion")
            .value(
                "no_information", EquivalenceCriterion::NoInformation,
                "No information on the equivalence is available. This can be "
                "due "
            ...
            .value("equivalent_up_to_phase",
                   ...
                   "Circuits are equivalent up to a certain (global or "
                   "relative) phase.")
            ...
            .value(
                ...
                "whenever a couple of simulations did not show the "
                "non-equivalence "
                "in the :ref:`simulation checker "
                "<EquivalenceChecking:Simulation "
                "Equivalence Checker (using Decision Diagrams)>`.")
            .value("probably_not_equivalent",
                   EquivalenceCriterion::ProbablyNotEquivalent,
                   "Circuits are probably not equivalent. A result that is "
                   "obtained "
                   "whenever the :ref:`ZX-calculus checker "
                   "<EquivalenceChecking:ZX-Calculus Equivalence Checker>` "
                   "could not "
        ...
        py::class_<Configuration> configuration(
            m, "Configuration",
            "Configuration options for the QCEC quantum circuit equivalence "
            "checking "
            "tool");
        ...
        ecm.def(py::init(&createManagerFromConfiguration), "circ1"_a, "circ2"_a,
            ...
            .def("get_configuration",
                 &EquivalenceCheckingManager::getConfiguration)
            ...
            .def("set_tolerance", &EquivalenceCheckingManager::setTolerance,
                 ...
                 "<.Configuration.Execution.numerical_tolerance>` of the "
                 "underlying "
            ...
            .def("set_simulation_checker",
                 &EquivalenceCheckingManager::setSimulationChecker,
                 "enable"_a = true,
            ...
            .def("reconstruct_swaps",
                 &EquivalenceCheckingManager::reconstructSWAPs,
                 ":attr:`Try to reconstruct SWAP gates "
                 "<.Configuration.Optimizations.reconstruct_swaps>` that have "
                 "been "
                 "decomposed or optimized away.")
            .def("reorder_operations",
                 &EquivalenceCheckingManager::reorderOperations,
                 ":attr:`Reorder operations "
                 "<.Configuration.Optimizations.reorder_operations>` to "
                 "establish "
                 "canonical ordering.")
            .def("backpropagate_output_permutation",
                 ...
                 "<.Configuration.Optimizations.backpropagate_output_"
                 "permutation>` "
            ...
            .def("set_application_scheme",
                 ...
                 "Set the :class:`Application Scheme <.ApplicationScheme>` "
                 "that is "
            ...
            .def("set_gate_cost_profile",
                 &EquivalenceCheckingManager::setGateCostProfile,
                 "profile"_a = "",
                 "Set the :attr:`profile <.Configuration.Application.profile>` "
                 "used "
                 "in the :attr:`Gate Cost <.ApplicationScheme.gate_cost>` "
                 "application scheme for all checkers (based on decision "
                 "diagrams).")
            ...
            .def("set_trace_threshold",
                 &EquivalenceCheckingManager::setTraceThreshold,
                 "threshold"_a = 1e-8,
                 "Set the :attr:`trace threshold "
                 "<.Configuration.Functionality.trace_threshold>` used for "
                 "comparing "
            ...
            .def("set_fidelity_threshold",
                 ...
                 "<.Configuration.Simulation.fidelity_threshold>` used for "
                 "comparing "
            ...
            .def("set_state_type", &EquivalenceCheckingManager::setStateType,
                 ...
                 "<.Configuration.Simulation.state_type>` used for the "
                 "simulations "
                 "in the :ref:`simulation checker "
                 "<EquivalenceChecking:Simulation "
                 "Equivalence Checker (using Decision Diagrams)>`.")
            .def(
                "set_seed", &EquivalenceCheckingManager::setSeed, "seed"_a = 0U,
            ...
            .def("store_cex_input", &EquivalenceCheckingManager::storeCEXinput,
                 ...
                 "<.Configuration.Simulation.store_cex_input>` if a "
                 "counterexample "
                 "is obtained.")
            .def("store_cex_output",
                 &EquivalenceCheckingManager::storeCEXoutput,
                 "enable"_a = false,
            ...
            .def("get_results", &EquivalenceCheckingManager::getResults,
                 "Returns the :class:`.EquivalenceCheckingManager.Results` of "
                 "the "
                 "equivalence check including statistics.")
            .def("json", &EquivalenceCheckingManager::json,
                 "Returns a JSON-style dictionary of all the information "
                 "present in "
                 "the :class:`.EquivalenceCheckingManager`")
            .def(
                "__repr__", &EquivalenceCheckingManager::toString,
        ...
        results.def(py::init<>())
            ...
            .def_readwrite(
                ...
                "Number of circuit instantiations that have been performed "
                "during "
            ...
            .def("considered_equivalent",
                 ...
                 "Convenience function to check whether the obtained result is "
                 "to be "
        ...
        py::class_<Configuration::Execution> execution(
            ...
            ":meth:`~.EquivalenceCheckingManager.run` "
            "method.");
        py::class_<Configuration::Optimizations> optimizations(
            configuration, "Optimizations",
            "Options that influence which circuit optimizations are applied "
            "during "
            "pre-processing.");
        ...
        py::class_<Configuration::Functionality> functionality(
            ...
            "Options for all checkers that consider the whole functionality of "
            "a "
            "circuit.");
        ...
        configuration.def(py::init<>())
            ...
            .def(
                "__repr__", &Configuration::toString,
        ...
        execution.def(py::init<>())
            ...
            .def_readwrite("nthreads", &Configuration::Execution::nthreads,
                           "Set the maximum number of threads to use. Defaults "
                           "to the maximum "
                           "number of available threads reported by the OS.")
            .def_readwrite(
                "timeout", &Configuration::Execution::timeout,
                "Set a timeout for :meth:`~.EquivalenceCheckingManager.run` "
                "(in "
            ...
            .def_readwrite(
                ...
                "Set whether the :ref:`construction checker "
                "<EquivalenceChecking:Construction Equivalence Checker (using "
                "Decision Diagrams)>` should be executed. Defaults to "
                ":code:`False` "
            ...
            .def_readwrite("run_zx_checker",
                           &Configuration::Execution::runZXChecker,
                           "Set whether the :ref:`ZX-calculus checker "
                           "<EquivalenceChecking:ZX-Calculus Equivalence "
                           "Checker>` should be "
                           "executed. Defaults to :code:`True` but arbitrary "
                           "multi-controlled "
        ...
        optimizations.def(py::init<>())
            ...
            .def_readwrite(
                ...
                "Try to reconstruct SWAP gates that have been decomposed (into "
                "a "
                "sequence of 3 CNOT gates) or optimized away (as a consequence "
                "of a "
                "SWAP preceded or followed by a CNOT on the same qubits). "
                "Defaults "
                "to :code:`True` since this reconstruction enables the "
                "efficient "
            ...
            .def_readwrite(
                ...
                "Remove any diagonal gates at the end of the circuit. This "
                "might be "
                "desirable since any diagonal gate in front of a measurement "
                "does "
                "not influence the probabilities of the respective states. "
                "Defaults "
                "to :code:`False` since, in general, circuits differing by "
                "diagonal "
            ...
            .def_readwrite(
                ...
                "Circuits containing dynamic circuit primitives such as "
                "mid-circuit "
                "measurements, resets, or classically-controlled operations "
                "cannot "
                "be verified in a straight-forward fashion due to the "
                "non-unitary "
                "nature of these primitives, which is why this setting "
                "defaults to "
                ":code:`False`. By enabling this optimization, any dynamic "
                "circuit "
                "is first transformed to a circuit without non-unitary "
                "primitives "
                "by, first, substituting qubit resets with new qubits and, "
                "then, "
                "applying the deferred measurement principle to defer "
                "measurements "
                "to the end.")
            .def_readwrite(
                "reorder_operations",
                &Configuration::Optimizations::reorderOperations,
                "The operations of a circuit are stored in a sequential "
                "container. "
                "This introduces some dependencies in the order of operations "
                "that "
                "are not naturally present in the quantum circuit. As a "
                "consequence, "
                "two quantum circuits that contain exactly the same "
                "operations, list "
                "their operations in different ways, also apply there "
                "operations in "
                "a different order. This optimization pass established a "
                "canonical "
                "ordering of operations by, first, constructing a directed, "
                "acyclic "
            ...
            .def_readwrite(
                ...
                "Backpropagate the output permutation to the input "
                "permutation. "
                "Defaults to :code:`False` since this might mess up the "
                "initially "
                "given input permutation. Can be helpful for dynamic quantum "
                "circuits"
                " that have been transformed to a static circuit by enabling "
                "the "
        ...
        application.def(py::init<>())
            .def_readwrite(
                ...
                "The :class:`Application Scheme <.ApplicationScheme>` used for "
                "the "
            ...
            .def_readwrite(
                ...
                "The :attr:`Gate Cost <.ApplicationScheme.gate_cost>` "
                "application "
                "scheme can be configured with a profile that specifies the "
                "cost of "
                "gates. At the moment, this profile can be set via a file that "
                "is "
                "constructed similar to a lookup table. Every line "
                ":code:`<GATE_ID> "
                "<N_CONTROLS> <COST>` specified the cost for a given gate type "
                "and "
                "with a certain number of controls, e.g., :code:`X 0 1` "
                "denotes that "
                "a single-qubit X gate has a cost of :code:`1`, while :code:`X "
                "2 15` "
        ...
        functionality.def(py::init<>())
            .def_readwrite(
                ...
                "While decision diagrams are canonical in theory, i.e., "
                "equivalent "
                "circuits produce equivalent decision diagrams, numerical "
                "inaccuracies and approximations can harm this property. This "
                "can "
                "result in a scenario where two decision diagrams are really "
                "close "
                "to one another, but cannot be identified as such by standard "
                "methods (i.e., comparing their root pointers). Instead, for "
                "two "
                "decision diagrams :code:`U` and :code:`U'` representing the "
                "functionalities of two circuits :code:`G` and :code:`G'`, the "
                "trace "
                "of the product of one decision diagram with the inverse of "
                "the "
                "other can be computed and compared to the trace of the "
                "identity. "
                "Alternatively, it can be checked, whether :code:`U*U`^-1` is "
                "\"close enough\" to the identity by recursively checking that "
                "each "
                "decision diagram node is close enough to the identity "
                "structure "
                "(i.e., the first and last successor have weights close to "
                "one, "
                "while the second and third successor have weights close to "
                "zero). "
                "Whenever any decision diagram node differs from this "
                "structure by "
                "more than the configured threshold, the circuits are "
                "concluded to "
        ...
        simulation.def(py::init<>())
            .def_readwrite(
                ...
                "<.Configuration.Functionality.trace_threshold>`, this setting "
                "is "
                "here to tackle numerical inaccuracies and approximations for "
                "the "
                ":ref:`simulation checker <EquivalenceChecking:Simulation "
                "Equivalence Checker (using Decision Diagrams)>`. Instead of "
                "computing a trace, the fidelity between the states resulting "
                "from "
                "the simulation is computed. Whenever the fidelity differs "
                "from "
                ":code:`1.` by more than the configured threshold, the "
                "circuits are "
            ...
            .def_readwrite(
                ...
                ":ref:`simulation checker <EquivalenceChecking:Simulation "
                "Equivalence Checker (using Decision Diagrams)>`. In practice, "
                "just "
                "a couple of simulations suffice in most cases to detect a "
                "potential "
            ...
            .def_readwrite("state_type", &Configuration::Simulation::stateType,
                           "The :class:`type of states <.StateType>` used for "
                           "the simulations "
                           "in the :ref:`simulation checker "
                           "<EquivalenceChecking:Simulation "
                           "Equivalence Checker (using Decision Diagrams)>`.")
            ...
            .def_readwrite(
                ...
                "Whether to store the input state that has lead to the "
                "determination "
                "of a counterexample. Since the memory required to store a "
                "full "
                "representation of a quantum state increases exponentially, "
                "this is "
            ...
            .def_readwrite(
                ...
                "non-equivalence of both circuits. Since the memory required "
                "to "
        ...
        parameterized.def(py::init<>())
            ...
            .def_readwrite(
                ...
                "Number of instantiations shall be performed in addition to "
                "the "
                "default ones. "
                "For parameterized circuits that cannot be shown to be "
                "equivalent "
                "by the ZX checker "
                "the circuits are instantiated with concrete values for "
                "parameters "
                "and subsequently "
                ...
                "parameters with "
                "random values to guarantee completeness of the equivalence "
                "check. "
                "Because random "
