...
namespace ec {
    void EquivalenceCheckingManager::setupAncillariesAndGarbage() {
        ...
        // reverse iterate over the removed qubits and add them back into the
        // larger circuit as ancillary
    ...
    void EquivalenceCheckingManager::fixOutputPermutationMismatch() {
        ...
        for (const auto& [lPhysical, lLogical] : largerOutput) {
            ...
            // algorithm has logical qubit that does not exist in the smaller
            // circuit
            if (!existsInSmaller) {
                continue;
            }
    ...
    void EquivalenceCheckingManager::runOptimizationPasses() {
        ...
        const auto isDynamicCircuit1 =
            qc::CircuitOptimizer::isDynamicCircuit(qc1);
        const auto isDynamicCircuit2 =
            qc::CircuitOptimizer::isDynamicCircuit(qc2);
        if (isDynamicCircuit1 || isDynamicCircuit2) {
            if (configuration.optimizations.transformDynamicCircuit) {
                ...
                throw std::runtime_error("One of the circuits contains "
                                         "mid-circuit non-unitary primitives. "
                                         "To verify such circuits, the checker "
                                         "must be configured with "
                                         "`transformDynamicCircuit=true` "
                                         "(`transform_dynamic_circuits=True` "
                                         "in Python).");
            }
        }
    ...
    EquivalenceCheckingManager::EquivalenceCheckingManager(
        const qc::QuantumComputation& circ1,
        const qc::QuantumComputation& circ2, Configuration config)
        : qc1(circ1.size() > circ2.size() ? circ2 : circ1),
          qc2(circ1.size() > circ2.size() ? circ1 : circ2),
          configuration(std::move(config)) {
        // clones both circuits (the circuit with fewer gates always gets to be
        // qc1)
        ...
        // given that one circuit has more qubits than the other, the difference
        // is assumed to arise from ancillary qubits. adjust both circuits
        // accordingly
        ...
        if (this->qc1.getNqubitsWithoutAncillae() !=
            ...
            std::clog
                << "[QCEC] Warning: circuits have different number of primary "
        ...
        // check whether the alternating checker is configured and can handle
        // the circuits
        if (configuration.execution.runAlternatingChecker &&
            !DDAlternatingChecker::canHandle(this->qc1, this->qc2)) {
            std::clog
                << "[QCEC] Warning: alternating checker cannot handle the "
        ...
        if (configuration.execution.runSimulationChecker &&
            configuration.simulation.stateType ==
    ...
    void EquivalenceCheckingManager::checkSequential() {
        ...
        if (configuration.execution.timeout > 0.) {
            timeoutThread =
                std::thread([this, timeout = std::chrono::duration<double>(
                                       configuration.execution.timeout)] {
                    std::unique_lock doneLock(doneMutex);
                    const auto       finished = doneCond.wait_for(
                        doneLock, timeout, [this] { return done; });
        ...
        if (configuration.execution.runSimulationChecker) {
            ...
            while (!simulationsFinished() && !done) {
                ...
                if (result == EquivalenceCriterion::NoInformation) {
                    if (!done) {
                        std::clog
                            << "Simulation run returned without any "
                               "information. "
            ...
            if (results.equivalence == EquivalenceCriterion::NotEquivalent) {
                ...
                if (configuration.simulation.storeCEXoutput) {
                    results.cexOutput1 =
                        simulationChecker->getInternalVector1();
                    results.cexOutput2 =
                        simulationChecker->getInternalVector2();
                }
        ...
        if (configuration.execution.runAlternatingChecker && !done) {
            checkers.emplace_back(std::make_unique<DDAlternatingChecker>(
        ...
        if (configuration.execution.runConstructionChecker && !done) {
            checkers.emplace_back(std::make_unique<DDConstructionChecker>(
                qc1, qc2, configuration));
        ...
        if (configuration.execution.runZXChecker && !done) {
            if (zx::FunctionalityConstruction::transformableToZX(&qc1) &&
                ...
                if (!done) {
                    ...
                    // no matter the result, everything is done as this is the
                    // last check
                    done = true;
                    doneCond.notify_one();
                    ...
                    if (result == EquivalenceCriterion::Equivalent ||
                        ...
                        if (results.equivalence ==
                            ...
                            std::clog << "The ZX checker suggests that the "
                                         "circuits are not "
                                         "equivalent, but the simulation "
                                         "checker suggests that "
                                         "they are probably equivalent. Thus, "
                                         "no conclusion "
                                         "can be drawn.\n";
                            results.equivalence =
                        ...
                        if (results.equivalence ==
                            EquivalenceCriterion::NoInformation) {
                            // this can only happen if the ZX checker is the
                            // only checker
                            assert(configuration.onlyZXCheckerConfigured());
                            std::clog << "Only ZX checker specified, but it "
                                         "was not able to conclude "
                                         "anything about the equivalence of "
                                         "the circuits!\n"
                                      << "This can happen since the ZX checker "
                                         "is not complete in "
                                         "general.\n"
                                      << "Consider enabling other checkers to "
                                         "get more "
                                         "information.\n";
                        }
                    }
                ...
                std::clog
                    << "Only ZX checker specified, but one of the circuits "
                       "contains "
    ...
    void EquivalenceCheckingManager::checkParallel() {
        ...
        std::chrono::time_point<std::chrono::steady_clock,
            ...
            deadline{};
        if (configuration.execution.timeout > 0.) {
            deadline = start + std::chrono::duration<double>(
                                   configuration.execution.timeout);
        }

        if (const auto threadLimit = std::thread::hardware_concurrency();
            threadLimit != 0U &&
            configuration.execution.nthreads > threadLimit) {
            std::clog
                << "Trying to use more threads than the underlying "
                   "architecture claims "
        ...
        // create a thread safe queue which is used to check for available
        // results
        ...
        if (configuration.execution.runAlternatingChecker) {
            ...
            futures.emplace_back(
                asyncRunChecker<DDAlternatingChecker>(id, queue));
            ++id;
        }

        if (configuration.execution.runConstructionChecker && !done) {
            // start a new thread that constructs and runs the construction
            // check
            futures.emplace_back(
                asyncRunChecker<DDConstructionChecker>(id, queue));
            ++id;
        }
        ...
        if (configuration.execution.runZXChecker && !done) {
            ...
            futures.emplace_back(
                asyncRunChecker<ZXEquivalenceChecker>(id, queue));
            ++id;
        }

        if (configuration.execution.runSimulationChecker) {
            const auto effectiveThreadsLeft = effectiveThreads - futures.size();
            const auto simulationsToStart   = std::min(
                effectiveThreadsLeft, configuration.simulation.maxSims);
            // launch as many simulations as possible
            for (std::size_t i = 0; i < simulationsToStart && !done; ++i) {
                futures.emplace_back(
                    asyncRunChecker<DDSimulationChecker>(id, queue));
                ++id;
                ++results.startedSimulations;
            }
        }
        ...
        while (!done) {
            ...
            // in case no completed ID has been returned this indicates a
            // timeout and the computation should stop
            if (!completedID) {
                setAndSignalDone();
                break;
            }
            ...
            // in case non-equivalence has been shown, the execution can be
            // stopped
            const auto* const checker = checkers.at(*completedID).get();
            const auto        result  = checker->getEquivalence();

            if (result == EquivalenceCriterion::NoInformation) {
                if (dynamic_cast<const ZXEquivalenceChecker*>(checker) !=
                    nullptr) {
                    if (configuration.onlyZXCheckerConfigured()) {
                        std::clog
                            << "Only ZX checker specified, but it was not able "
                               "to conclude "
                               "anything about the equivalence of the "
                               "circuits!\n"
                            << "This can happen since the ZX checker is not "
                               "complete in "
                               "general.\n"
                ...
                std::clog
                    << "Finished equivalence check provides no information. "
                       "Something probably went wrong. Exiting.\n";
                setAndSignalDone();
                results.equivalence = result;
                break;
            ...
            if (result == EquivalenceCriterion::NotEquivalent) {
                ...
                // some special handling in case non-equivalence has been shown
                // by a simulation run
                if (const auto* const simulationChecker =
                    ...
                    if (configuration.simulation.storeCEXinput) {
                        results.cexInput =
                            simulationChecker->getInitialVector();
                    }
                    if (configuration.simulation.storeCEXoutput) {
                        results.cexOutput1 =
                            simulationChecker->getInternalVector1();
            ...
            if ((dynamic_cast<const DDAlternatingChecker*>(checker) !=
                 nullptr) ||
                (dynamic_cast<const DDConstructionChecker*>(checker) !=
                 nullptr)) {
                setAndSignalDone();
                results.equivalence = result;
                break;
            ...
            if (dynamic_cast<const ZXEquivalenceChecker*>(checker) != nullptr) {
                ...
                if (result == EquivalenceCriterion::ProbablyNotEquivalent) {
                    ...
                    if (results.equivalence ==
                        ...
                        // Since the ZX checker is not complete, it cannot
                        // conclude non-equivalence, but only suggest it. If the
                        // ZX checker is not the only checker configured to run,
            ...
            // at this point, the only option is that this is a simulation
            // checker
            if (dynamic_cast<const DDSimulationChecker*>(checker) != nullptr) {
                ++results.performedSimulations;

                // if no information is known, the successful simulation
                // suggests that both circuits are likely to be equivalent.
                if (results.equivalence ==
                    EquivalenceCriterion::NoInformation) {
                    results.equivalence =
                        EquivalenceCriterion::ProbablyEquivalent;
                ...
                if (simulationsFinished()) {
                    if (configuration.onlySimulationCheckerConfigured()) {
                        // if only simulations are performed and all of them are
                        // successful, the circuits are most likely equivalent,
                    ...
                    if (results.equivalence ==
                        ...
                        std::clog << "The ZX checker suggests that the "
                                     "circuits are not "
                                     "equivalent, but the simulation checker "
                                     "suggests that they "
                                     "are probably equivalent. Thus, no "
                    ...
                    // if all simulations finished and none of them showed
                    // non-equivalence, the run continues uninterrupted.
                    continue;
                }

                // it has to be checked, whether further simulations shall be
                // conducted
                if (results.startedSimulations <
                    configuration.simulation.maxSims) {
                    futures[*completedID] =
                        asyncRunChecker<DDSimulationChecker>(*completedID,
                                                             queue);
                    ++results.startedSimulations;
                }
            }
        ...
        // Futures are not explicitly waited for here, since the destructor of
        // the `std::future` object will block until the associated thread has
        // finished. If any thread is still stuck in a long-running operation,
        // this might take a while, but the program will terminate anyway. C++20
        // introduces `std::jthread`, which allows to explicitly cancel a
        // thread. This could be a solution for the future to avoid this problem
        // (and reduce the number of `isDone` checks).
    }
    ...
    void EquivalenceCheckingManager::checkSymbolic() {
        ...
        if (configuration.execution.timeout > 0.) {
            timeoutThread =
                std::thread([this, timeout = std::chrono::duration<double>(
                                       configuration.execution.timeout)] {
                    std::unique_lock doneLock(doneMutex);
                    auto finished = doneCond.wait_for(doneLock, timeout,
                                                      [this] { return done; });
                    // if the thread has already finished within the timeout,
                    // nothing has to be done
                    if (!finished) {
        ...
        if (!done) {
            if (zx::FunctionalityConstruction::transformableToZX(&qc1) &&
                ...
                std::clog
                    << "Checking symbolic circuits requires transformation "
    ...
    nlohmann::json EquivalenceCheckingManager::Results::json() const {
        ...
        if (startedSimulations > 0) {
            ...
            if (!cexInput.empty() || !cexOutput1.empty() ||
                !cexOutput2.empty()) {
