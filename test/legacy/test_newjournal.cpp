...
protected:
    ...
    static bool setExists(const std::set<std::set<std::uint64_t>>& all,
        ...
        for (const auto& set : all) {
            bool equals = true;
            for (auto setit = set.begin(), testit = test.begin();
                 setit != set.end(); ++setit, ++testit) {
                if (*setit != *testit) {
                    equals = false;
                    break;
                }
    ...
    void SetUp() override {
        std::stringstream ss{};
        ss << testTranspiledDir << std::get<0>(GetParam())
           << "_transpiled.qasm";
        ...
        std::generate(begin(randomData), end(randomData),
                      [&]() { return rd(); });
    ...
    void addToStatistics(std::uint16_t tryCount, double time,
                         std::size_t nsims) {
        if (tryCount == 0U) {
            minTime = time;
            maxTime = time;
...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const testing::TestParamInfo<NewJournalTestNonEQ::ParamType>& inf) {
...
TEST_P(NewJournalTestNonEQ, PowerOfSimulation) {
...
TEST_P(NewJournalTestNonEQ, PowerOfSimulationParallel) {
    config.execution.runAlternatingChecker  = false;
    config.execution.runConstructionChecker = false;
    config.execution.runZXChecker           = false;
    config.execution.runSimulationChecker   = true;
...
class NewJournalTestEQ : public testing::TestWithParam<std::string> {
protected:
    qc::QuantumComputation qcOriginal;
    qc::QuantumComputation qcTranspiled;
    ec::Configuration      config{};
    ...
    void SetUp() override {
        ...
        config.application.simulationScheme =
...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const testing::TestParamInfo<NewJournalTestEQ::ParamType>& inf) {
...
TEST_P(NewJournalTestEQ, EQReference) {
...
TEST_P(NewJournalTestEQ, EQNaive) {
...
TEST_P(NewJournalTestEQ, EQProportional) {
    config.execution.runAlternatingChecker = true;
...
TEST_P(NewJournalTestEQ, EQLookahead) {
    config.execution.runAlternatingChecker = true;
...
TEST_P(NewJournalTestEQ, EQPowerOfSimulation) {
    config.execution.runSimulationChecker = true;
...
TEST_P(NewJournalTestEQ, EQReferenceParallel) {
    config.execution.parallel               = true;
    config.execution.runConstructionChecker = true;
    config.execution.runSimulationChecker =
        true; // additionally run simulations
    config.application.constructionScheme = ec::ApplicationSchemeType::OneToOne;
...
TEST_P(NewJournalTestEQ, EQNaiveParallel) {
    config.execution.parallel              = true;
    config.execution.runAlternatingChecker = true;
    config.application.alternatingScheme = ec::ApplicationSchemeType::OneToOne;
...
TEST_P(NewJournalTestEQ, EQProportionalParallel) {
    config.execution.parallel              = true;
    config.execution.runAlternatingChecker = true;
    config.application.alternatingScheme =
        ec::ApplicationSchemeType::Proportional;
...
TEST_P(NewJournalTestEQ, EQLookaheadParallel) {
    config.execution.parallel              = true;
    config.execution.runAlternatingChecker = true;
    config.application.alternatingScheme = ec::ApplicationSchemeType::Lookahead;
...
TEST_P(NewJournalTestEQ, EQPowerOfSimulationParallel) {
    config.execution.parallel             = true;
    config.execution.runSimulationChecker = true;
