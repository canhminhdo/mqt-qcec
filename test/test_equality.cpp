...
TEST_F(EqualityTest, AutomaticSwitchToConstructionChecker) {
    ...
    // Check an exception is raised for a checker configured after
    // initialization. Note: this exception can only be caught in sequential
    // mode since it is raised in a different thread otherwise.
    ecm.reset();
