...
namespace ec {
    ...
    template <class DDType, class Config = dd::DDPackageConfig>
    class TaskManager {
    ...
    public:
        ...
        const std::unique_ptr<qc::Operation>& operator()() const {
            return *iterator;
        }
        ...
        [[nodiscard]] const qc::QuantumComputation*
        getCircuit() const noexcept {
            return qc;
        }
        ...
        void changePermutation(DDType& state) {
            dd::changePermutation(state, permutation, qc->outputPermutation,
                                  package, static_cast<bool>(direction));
        }
