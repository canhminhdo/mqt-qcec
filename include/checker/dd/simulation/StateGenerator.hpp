      ...
      public:
        ...
        qc::VectorDD generateRandomState(
            ...
            case ec::StateType::Random1QBasis:
                return generateRandom1QBasisState(dd, totalQubits,
                                                  ancillaryQubits);
            case ec::StateType::Stabilizer:
                return generateRandomStabilizerState(dd, totalQubits,
                                                     ancillaryQubits);
            default:
        ...
        qc::VectorDD generateRandomComputationalBasisState(
            ...
            if (constexpr auto bitwidth =
                ...
                const auto maxStates = static_cast<std::uint64_t>(1U)
                                       << randomQubits;
                ...
                while (!success) {
                    std::tie(randomState, success) =
                        generatedComputationalBasisStates.insert(
                            distribution(mt));
                }
                ...
                for (std::size_t i = 0U; i < randomQubits; ++i) {
                    if ((*randomState &
                         (static_cast<std::uint64_t>(1U) << i)) != 0U) {
                        stimulusBits[i] = true;
                    }
                }
            } else {
        ...
        generateRandom1QBasisState(std::unique_ptr<DDPackage>& dd,
            ...
            auto randomBasisState = std::vector<dd::BasisStates>(
                totalQubits, dd::BasisStates::zero);
            for (std::size_t i = 0U; i < randomQubits; ++i) {
        ...
        generateRandomStabilizerState(std::unique_ptr<DDPackage>& dd,
            ...
            auto rcs = qc::RandomCliffordCircuit(
                randomQubits,
                static_cast<std::size_t>(std::round(std::log2(randomQubits))),
                mt());

            // generate the associated stabilizer state by simulating the
            // Clifford circuit
            auto stabilizer =
                simulate(&rcs, dd->makeZeroState(randomQubits), dd);

            // decrease the ref count right after so that it stays correct later
            // on
            ...
            for (std::size_t p = randomQubits; p < totalQubits; ++p) {
                initial =
                    dd->makeDDNode(static_cast<dd::Qubit>(p),
