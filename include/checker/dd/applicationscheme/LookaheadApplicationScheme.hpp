...
public:
    ...
    // in general, the lookup application scheme will apply a single
    // operation of either circuit for every invocation. manipulation of the
    // state is handled directly by the application scheme. Thus, the return
    // value is always {0,0}.
    std::pair<size_t, size_t> operator()() override {
        ...
        // no operations shall be applied by the outer loop in which the
        // application scheme is invoked
        return {0U, 0U};
    }
...
private:
    ...
    // the lookahead application scheme maintains links to an internal state
    // to manipulate and a package to use
