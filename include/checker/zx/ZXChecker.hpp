...
namespace ec {
    class ZXEquivalenceChecker : public EquivalenceChecker {
      ...
      private:
        ...
        // routines that additionally check a criterion for early termination of
        // the simplification.
        ...
        std::size_t simplifyEdges(CheckFun check, RuleFun rule) {
            ...
            while (!isDone() && newMatches) {
                ...
                for (const auto& [v0, v1] : miter.getEdges()) {
                    if (isDone() || miter.isDeleted(v0) ||
                        miter.isDeleted(v1) || !check(miter, v0, v1)) {
                        continue;
                    }
                    rule(miter, v0, v1);
                    newMatches = true;
