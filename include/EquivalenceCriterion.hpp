...
namespace ec {
    ...
    inline std::string
    toString(const EquivalenceCriterion& criterion) noexcept {
        switch (criterion) {
    ...
    inline EquivalenceCriterion
    fromString(const std::string& criterion) noexcept {
        ...
        if ((criterion == "equivalent_up_to_global_phase") ||
            (criterion == "3")) {
