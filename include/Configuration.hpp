    ...
    class Configuration {
      public:
        ...
        struct Simulation {
            ...
            // this function makes sure that the maximum number of simulations
            ...
            static std::size_t computeMaxSims() {
                ...
                // catch the case where hardware_concurrency() returns 0 or the
                // other pre-configured checkers already use up all the
                // available threads
        ...
        [[nodiscard]] bool onlySingleTask() const noexcept {
            ...
            if (execution.runSimulationChecker && (simulation.maxSims == 1U) &&
                !execution.runAlternatingChecker &&
                !execution.runConstructionChecker) {
                return true;
            }

            // no simulations and only one of the other checks shall be
            // performed
        ...
        [[nodiscard]] nlohmann::json json() const {
            ...
            opt["transform_dynamic_circuit"] =
                optimizations.transformDynamicCircuit;
            ...
            if (execution.runConstructionChecker) {
                app["construction"] =
                    ec::toString(application.constructionScheme);
            }
            if (execution.runSimulationChecker) {
            ...
            if (execution.runAlternatingChecker) {
                app["alternating"] =
                    ec::toString(application.alternatingScheme);
            }
            if ((application.constructionScheme ==
                 ApplicationSchemeType::GateCost) ||
                (application.simulationScheme ==
                 ApplicationSchemeType::GateCost) ||
            ...
            par["additional_instantiations"] =
                parameterized.nAdditionalInstantiations;
