...
namespace ec {
    ...
    class EquivalenceCheckingManager {
      public:
        ...
        // convenience functions for changing the configuration after the
        // manager has been constructed: Execution: These settings may be
        // changed to influence what is executed during `run`
        ...
        void setZXChecker(bool run) {
            configuration.execution.runZXChecker = run;
        }
        ...
        // Optimization: Optimizations are applied during initialization.
        // Already configured and applied optimizations cannot be reverted
        ...
        void
        setApplicationScheme(const ApplicationSchemeType applicationScheme) {
        ...
        // Simulation: These setting may be changed to adjust the kinds of
      ...
      protected:
        ...
        std::future<void> asyncRunChecker(const std::size_t             id,
            ...
            return std::async(std::launch::async, [this, id, &queue]() {
                try {
                    ...
                    if (!checker) {
                        checker =
                            std::make_unique<Checker>(qc1, qc2, configuration);
                    }

                    if constexpr (std::is_same_v<Checker,
                                                 DDSimulationChecker>) {
                        auto* const simChecker =
                            dynamic_cast<DDSimulationChecker*>(checker.get());
                        const std::lock_guard stateGeneratorLock(
                            stateGeneratorMutex);
        ...
        [[nodiscard]] bool simulationsFinished() const {
            return results.performedSimulations ==
